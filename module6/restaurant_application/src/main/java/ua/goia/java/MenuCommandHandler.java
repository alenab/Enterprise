...
public class MenuCommandHandler implements CommandHandler {
    ...
    public MenuCommandHandler(MenuDao menuDao, DishDao dishDao) {
    ...
    public String handler(String... commands) {
        ...
        switch (command) {
            ...
            case "delete_dish":
                if (menuDao.deleteDish(Integer.valueOf(commands[1]), dishDao.findByName(commands[2]).get(0).getDishID()) > 0) {
                    return "deleted successfully";
                } else {
                    return "not deleted";
                }
