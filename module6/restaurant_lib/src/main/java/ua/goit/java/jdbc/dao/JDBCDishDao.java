...
public class JDBCDishDao implements DishDao {
    ...
    public List<Ingredients> getIngredientsList(int dishId) {
        ...
        try (Connection connection = DriverManager.getConnection(url, user, password);
            ...
            ResultSet resultSet = statement.executeQuery();
            while (resultSet.next()) {
                listOfIngredients.add(new JDBCIngredientsDao().getById(resultSet.getInt("INGREDIENT_ID")));
            }
        } catch (SQLException e) {
            throw new RuntimeException(e);
        }
    ...
    private Dish createDish(ResultSet resultSet) throws SQLException {
        ...
        Category category = new JDBCCategoryDao().getById(resultSet.getInt("category_id"));
