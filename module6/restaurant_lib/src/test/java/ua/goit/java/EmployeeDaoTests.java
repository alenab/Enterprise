...
public class EmployeeDaoTests {
    ...
    public void checkGetAll() {
        ...
        for (Employee item : list) {
            ...
            assertThat(item.getPositionId(), greaterThan(0));
        }
    }
    ...
    public void checkAdd() {
        ...
        int result = employeeDao.add(surname, name, birthday, null, positionId, salary);
        ...
        for (Employee item : list) {
            if (item.getName().equals(name) && item.getSurname().equals(surname)) {
                ...
                assertThat(item.getPositionId(), equalTo(positionId));
